...
const CATALOG = [
  ...
  { id: "daily", category: "rituals", icon: "flash", title: "Momentjäger", metric: "dailyJoins", tiers: [1, 10, 50], text: (t) => (t === 1 ? "Beim Call Me Moment dabei gewesen" : `${n(t)}× beim Call Me Moment dabei`) },
  { id: "blitz", category: "rituals", icon: "rocket", title: "Blitzstart", metric: "blitz", tiers: [1], secret: true, text: () => "In der ersten Minute beim Call Me Moment dabei" },
  // Kreise
...
async function metricsOf(user) {
  ...
  const [talks, moments, dailyJoins, blitz, circlesFounded, roomsStarted, myCircles] = await Promise.all([
    talksOf(phone),
