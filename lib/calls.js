...
function createCallService(io, { ringTimeoutMs = RING_TIMEOUT_MS } = {}) {
   ...
   */
  async function startCall({ from, to, channel }) {
    ...
    let call;
    try {
      call = await Call.create({ callId, channel, caller: from, callee: to });
    } catch (error) {
    ...
    if (targetOnline) {
      io.to(roomOf(to)).emit("incomingCall", { callId, from, channel, callerName, timestamp: Date.now() });
    }

    // VoIP push (iOS, works in background); regular push as fallback
    let notified = await sendVoipPushNotification(from, to, channel, callerName, callId);
    if (!notified) {
      notified = await sendEnhancedCallNotification(from, to, channel, callerName, callId);
    }
  ...
  async function missCall(callId) {
    ...
    const [caller, callee] = await Promise.all([
      User.findOne({ phone: call.caller }, "name"),
      User.findOne({ phone: call.callee }, "pushToken"),
    ]);
    if (callee?.pushToken) {
      await sendExpoPushes([
        {
          ...
          body: `${caller?.name || "Ein Kontakt"} hat versucht, dich zu erreichen.`,
          data: { type: "missed_call", phone: call.caller },
        },
      ]);
    }
  }
  ...
  return { startCall, acceptCall, endCall, onRegister, sweepStaleCalls, missCall };
}
