...
const { localParts, weekKey, DEFAULT_TIMEZONE } = require("./localTime");
...
async function warmthOf(circle, now = new Date()) {
  ...
  const goalReached = members.length >= 2 && talkedCount === members.length;
  if (goalReached) {
    const week = weekKey(now, circle.ritual?.timezone || DEFAULT_TIMEZONE);
    if (!(circle.goalWeeks || []).includes(week)) {
      await Circle.updateOne({ _id: circle._id }, { $addToSet: { goalWeeks: week } });
    }
  }
  return {
...
async function openRoom(circle, startedBy, now = new Date()) {
  const existing = await activeRoomOf(circle._id);
  if (existing) return { room: existing, created: false };
  // How long a round may run follows the founder's plan
  const { limits } = await require("./plan").planOfPhone(circle.createdBy, now);
  const room = await Room.create({
    circleId: circle._id,
    channel: `room_${circle._id}_${Date.now().toString(36)}`,
    startedBy,
    endsAt: limits.roomMinutes ? new Date(now.getTime() + limits.roomMinutes * 60 * 1000) : null,
  });
...
async function leaveRoom(room, phone, now = new Date()) {
  ...
  if (overlapping.length && seconds > 0) {
    ...
    // A round counts like a call for unlocking the day's moments
    if (seconds >= 60) await require("./unlock").noteUnlockFor([phone], "talk", now);
  }
...
/**
 * Rooms left open by crashed apps, and rounds of free circles past their
 * time: everyone still inside leaves now. Returns the rooms ended by time.
 */
async function endStaleRooms(now = new Date()) {
  const stale = await Room.find({
    active: true,
    $or: [{ createdAt: { $lt: new Date(now - ROOM_MAX_MS) } }, { endsAt: { $ne: null, $lte: now } }],
  });
