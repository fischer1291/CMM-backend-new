...
const CATALOG = {
  contact_available: {
    ...
    skipIfInApp: true,
    throttle: ({ phone }) => ({ key: `available:${phone}`, ms: 10 * MINUTE }),
    content: ({ name }) => ({
...
/**
 * Which of these phones have the app open in the foreground right now.
 * Wired to Socket.IO in app.js (clients report "presence").
 */
let foregroundPhones = async () => new Set();
...
async function notifyMany(recipients, type, params, { now = new Date() } = {}) {
  ...
  const results = [];
  const messages = [];
  const inApp = CATALOG[type]?.skipIfInApp
    ? await foregroundPhones(users.filter(Boolean).map((u) => u.phone)).catch(() => new Set())
    : new Set();
  for (const user of users) {
    ...
    const prepared = await prepare(user, type, params, now, inApp.has(user.phone));
  ...
  const decisions = results
    .filter((r) => r.phone)
    .map((r) => ({ to: r.phone, type, about: params.phone, result: r.sent ? "sent" : r.skipped, at: now }));
