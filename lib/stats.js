...
async function statsFor(user, { timezone, now = new Date() } = {}) {
  ...
  const [talks, moments] = await Promise.all([
    // 1:1 talks the user took part in; group talks only the user's own record
  ...
  for (const talk of talks) {
    ...
    // Time with each person (in a group call: with everyone who was there)
    for (const other of talk.participants.filter((p) => p !== phone)) {
...
function sharedView(stats) {
  return {
    ...
    badges: stats.badges
      .filter((b) => b.earned)
      .map(({ id, title, description, icon, tierName }) => ({ id, title, description, icon, tierName })),
    // Up to three badges the owner chose to show
...
module.exports = { statsFor, sharedView, canView };
