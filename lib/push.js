...
// Logs show only the start and end of a number
const mask = (phone) => (phone ? `${String(phone).slice(0, 3)}…${String(phone).slice(-3)}` : "?");
...
async function sendVoipPushNotification(
  ...
  try {
    console.log(`📞 Attempting VoIP push: ${mask(callerPhone)} -> ${mask(calleePhone)}`);

    // Get callee's VoIP token
    const calleeUser = await User.findOne({ phone: calleePhone });
    if (!calleeUser || !calleeUser.voipToken) {
      console.log(`❌ No VoIP token found for user: ${mask(calleePhone)}`);
      return false;
    }
    ...
    for (const environment of environments) {
      ...
      if (result.sent && result.sent.length > 0) {
        ...
        console.log(`✅ VoIP push sent (${environment}) to: ${mask(calleePhone)}`);
        return true;
      }
      ...
      if (String(failure?.status) === "410" || reason === "Unregistered") {
        ...
        console.log(`🧹 Removed invalid VoIP token for user: ${mask(calleePhone)}`);
      }
      return false;
    }
...
async function sendEnhancedCallNotification(
  ...
  try {
    ...
    if (!calleeUser || !calleeUser.pushToken) {
      console.log(`❌ No push token found for user: ${mask(calleePhone)}`);
      return false;
    }
    ...
    if (!Expo.isExpoPushToken(pushToken)) {
      console.log(`❌ Invalid push token for user: ${mask(calleePhone)}`);
      return false;
    }
